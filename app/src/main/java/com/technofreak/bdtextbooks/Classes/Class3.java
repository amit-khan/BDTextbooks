...
public class Class3 extends AppCompatActivity {
    private static String [] file_url = {
            "https://drive.google.com/uc?export=download&id=0B8L6VJQZe3EEZ0R4dHdhNmVuM2c",
            "https://drive.google.com/uc?export=download&id=0BymczCMNoYu1MkNEcWhpU19BNE0",
            "https://drive.google.com/uc?export=download&id=0B8L6VJQZe3EEU3NXZ0ZjWkk4MkU",
            "https://drive.google.com/uc?export=download&id=0B8L6VJQZe3EESWQwSnktc3psXzg",
            "https://drive.google.com/uc?export=download&id=0B8L6VJQZe3EELTROTmN5STRKUEE",
            "https://drive.google.com/uc?export=download&id=0B8L6VJQZe3EEb1kxbGFhNDJnNDQ" };
    ...
    private class DownloadFileFromURL extends AsyncTask<String,String,String> {
        ...
        protected String doInBackground(String... f_url) {
            ...
            try {
                ...
                if (fileNo==0) {
                    output = new FileOutputStream(appDir + "/ban_3.pdf");
                } else if (fileNo==1){
                    output = new FileOutputStream(appDir + "/mat_3.pdf");
                }else if (fileNo==2){
                    output = new FileOutputStream(appDir + "/eng_3.pdf");
                }else if (fileNo==3){
                    output = new FileOutputStream(appDir + "/isl_3.pdf");
                }else if (fileNo==4){
                    output = new FileOutputStream(appDir + "/hin_3.pdf");
                }else if (fileNo==5){
                    output = new FileOutputStream(appDir + "/sci_3.pdf");
                }
    ...
    public void ban(View view) {
        File f = new File(appDir + "/ban_3.pdf");
        if (f.exists()){
            open(f);
        }else {
            fileNo=0;
    ...
    public void mat(View view) {
        File f = new File(appDir + "/mat_3.pdf");
        if (f.exists()){
            open(f);
        }else {
            fileNo=1;
    ...
    public void eng(View view) {
        File f = new File(appDir + "/eng_3.pdf");
        if (f.exists()){
            open(f);
        }else {
            fileNo=2;
    ...
    public void islam(View view) {
        File f = new File(appDir + "/isl_3.pdf");
        if (f.exists()){
            open(f);
        }else {
            fileNo=3;
    ...
    public void hindu(View view) {
        File f = new File(appDir + "/hin_3.pdf");
        if (f.exists()){
            open(f);
        }else {
            fileNo=4;
    ...
    public void sci(View view) {
        File f = new File(appDir + "/sci_3.pdf");
        if (f.exists()){
            open(f);
        }else {
            fileNo=5;
