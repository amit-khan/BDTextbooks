...
public class Class2 extends AppCompatActivity {
    private static String [] file_url = {
            "https://drive.google.com/uc?export=download&id=0B8L6VJQZe3EEbFgtVDZSdUd5dXc",
            "https://drive.google.com/uc?export=download&id=0BymczCMNoYu1UXRlSHltZy1xZFE",
            "https://drive.google.com/uc?export=download&id=0B8L6VJQZe3EEQ0tMTlE2UkZmcE0"};
    ...
    private class DownloadFileFromURL extends AsyncTask<String,String,String> {
        ...
        protected String doInBackground(String... f_url) {
            ...
            try {
                ...
                if (fileNo==0) {
                    output = new FileOutputStream(appDir + "/ban_2.pdf");
                } else if (fileNo==1){
                    output = new FileOutputStream(appDir + "/mat_2.pdf");
                }else if (fileNo==2){
                    output = new FileOutputStream(appDir + "/eng_2.pdf");
                }
    ...
    public void ban(View view) {
        File f = new File(appDir + "/ban_2.pdf");
        if (f.exists()){
            open(f);
        }else {
            fileNo=0;
    ...
    public void mat(View view) {
        File f = new File(appDir + "/mat_2.pdf");
        if (f.exists()){
            open(f);
        }else {
            fileNo=1;
    ...
    public void eng(View view) {
        File f = new File(appDir + "/eng_2.pdf");
        if (f.exists()){
            open(f);
        }else {
            fileNo=2;
